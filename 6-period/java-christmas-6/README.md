...
---
1:40 어느정도 스캐치하고, 목록 작성시작
2:13 기능 목록 작성 완료. 개발 준비 시작

---

## 예상 방문 날짜를 입력받는다.

#### 시작 메시지를 콘솔에 출력한다 ("12월 중 식당 예상 방문 날짜는 언제인가요? (숫자만 입력해 주세요!)" )

#### 숫자만 받는다

#### 방문할 날짜는 1 이상 31 이하의 숫자로만 입력받아야 한다.

#### "[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요."라는 에러 메시지를 보여 준다.

#### 방문날짜 객체로 표현한다.

#### 검증 실패 시 그 부분부터 재입력 하게 한다.

## 메뉴,개수를 여러개까지 입력받는다.

#### 시작 메시지를 콘솔 출력한다 ("주문하실 메뉴를 메뉴와 개수를 알려 주세요. (e.g. 해산물파스타-2,레드와인-1,초코케이크-1)")

#### ,를 구분자로 해서 (메뉴-수량) 으로 나눈다.

#### 입력 메뉴를, 크리스마스의 메뉴에서 찾을 수 있어야 한다.

#### 수량을 1개~100개까지 입력 받을 수 있어야 한다.
...
#### 4가지 메뉴 타입이 있고 다양한 메뉴들이 존재한다

#### 메뉴이름으로 메뉴를 생성해서 반환한다.

#### 메뉴이름으로 찾을 수 없을 시, 예외 반환한다.
...
##### - 조건 : 기간안에 포함된 경우 (2023.12.1 ~ 2023.12 .25)

##### - 금액 : 12.1일 1000원 시작, 하루 늘어날때마다 100원 할인

##### - 계산 : 총 주문 금액 - 해당 금액 만큼 할인

### 평일 할인

##### - 조건 : 기간안에 포함된 경우 (2023.12.1 ~ 2023.12.31 && 일~ 목 인경우)

##### - 금액 : 디저트 메뉴 1개당 2023원 할인

##### - 계산 : 총 주문 금액 - 해당 금액 만큼 할인

### 주말 할인

##### - 조건 : 기간안에 포함된 경우 (2023.12.1 ~ 2023.12.31 && 금,토 인경우)
...
##### - 조건 : 기간안에 포함된 경우 (2023.12.1 ~ 2023.12.31 && 다음 day인 경우(3,10,17,24,25,31)
...
#### 계산 끝난 Discount 와, 계산 끝난 Bage를 가지고 있다.

## OutputView

Receipe 내용을 바탕으로 콘솔에 출력한다.
