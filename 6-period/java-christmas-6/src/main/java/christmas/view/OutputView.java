...
public class OutputView {
    ...
    private static final String FORMAT_ORDER = "%s %d개";

    private static final String TITLE_TOTAL_ORDER_PRICE = "<할인 전 총주문 금액>";
    private static final String FORMAT_ORDER_PRICE = "%,d원";
    ...
    public void showTitleOrderMenu(final List<OrderDto> dtos) {
        ...
        dtos.stream()
                .map(dto -> format(FORMAT_ORDER, dto.name(), dto.quantity()))
                .forEach(this::showln);
    }

    public void showTitleTotalOrderPrice(final int price) {
        showln(LINE + TITLE_TOTAL_ORDER_PRICE);
        showln(format(FORMAT_ORDER_PRICE, price));
    }
    ...
    public void showTitleGiftMenu(final List<OrderDto> dtos) {
        ...
        showln("");
        dtos.stream()
                .map(dto -> format(TITLE_GIFT_MENU, dto.name(), dto.quantity()))
                .forEach(this::showln);
    }

    public void showTitleDisountDetails(final DiscountDetailsDto dto) {
        ...
        if (dto.specialDiscount() != 0) {
            showln(format(FORMAT_SPECIAL_DISCOUNT, dto.christmasDday()));
        }
        if (dto.giftEvent() != 0) {
    ...
    private void checkDayDiscount(final DiscountDetailsDto dto) {
        if (dto.dayDiscount() != 0) {
            if (dto.isWeekday()) {
                showln(format(FORMAT_WEEKDAY_DISCOUNT, dto.christmasDday()));
            }
            showln(format(FORMAT_WEEKEND_DISCOUNT, dto.christmasDday()));
        }
    }
