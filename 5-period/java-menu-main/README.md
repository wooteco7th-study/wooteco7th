...
- 한 주에 같은 카테고리는 최대 2회까지만 고를 수 있다. ex)구구: 비빔밥, 김치찌개, 쌈밥, 규동, 우동 → 한식을 3회 먹으므로 불가능
- 각 코치에게 한 주에 중복되지 않는 메뉴를 추천해야 한다. ex) 토미: 비빔밥, 비빔밥, 규동, 우동, 볶음면 → 한 코치가 같은 메뉴를 먹으므로 불가능
